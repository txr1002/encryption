...
int main(){
    ...
    cout<<"Please enter the number you want to enter and the plaintext ciphertext pair:"<<endl;
    ...
    cin>>texts;
    cin>>codes;
    if(solve(texts,codes) == true)//此时，k数组中已经储存可行的密钥,第一轮中tp数组储存了第一个对子可能的密钥值
        {
            if(i>0)
            {
                int a =0;
                ...
                }//两个数组都遍历完了，相同的密钥放在tp最后几位，接下来往前抬
                for(int i = 0;i<20;i++)
                {
                    if(a>0) 
                    {tp[i] = tp[19-i];
    ...
    int t=0;
    if(tp[0] == -1) cout<<"failure";
    else cout<<"The possible keys are:"<<endl;
    //给tp数组去重，放到temp里
    for(int i = 0;i<10;i++)
    {
        temp[i] = -1;
    ...
    for(int i = 0;i<20;i++)
    {
        if(tp[i]!= -1)
        {
            ...
            for(int j = 0;j<10;j++)
            {
                //cout<<"第"<<i<<"tp:"<<tp[i]<<" temp:"<<temp[j];
                if(tp[i] == temp[j]) flag = false;
            }
            if(flag == true)
            {
                temp[t]=tp[i];
